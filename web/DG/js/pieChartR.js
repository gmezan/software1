...
for(i = 0; i < document.getElementById("dataTable").rows.length -2; i++ ){
  ...
  if (document.getElementById("dataTable").rows[i+1].cells[2].innerHTML.toLowerCase() ==='egresado'){
      ...
      donacion_egresados += parseInt(document.getElementById("dataTable").rows[i+1].cells[3].innerHTML);
  }
  else{
      donacion_alumnos += parseInt(document.getElementById("dataTable").rows[i+1].cells[3].innerHTML);
  }
